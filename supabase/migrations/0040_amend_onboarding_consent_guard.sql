-- PHLOEM migration 0040_amend_onboarding_consent_guard.sql
--
-- Additive in effect: replaces amend_onboarding (0039, applied the same day and
-- not yet called by any code on main) with one more refused key.
--
-- `consent` is the family's own tick under the "How we use this information"
-- notice. An amendment must never change it: that would falsify the record of
-- what the family agreed to. Refused in the RPC, not merely hidden in the form.
...
begin
  ...
  for v_key in select jsonb_object_keys(p_patch) loop
    if v_key = any(array[
      ...
      -- the family's own data-processing consent: never amended on their behalf
      'consent'
    ]) then
