...
export default async function CoordinatorMemberPage({
  ...
  const [{ data: contacts }, { data: consults }, { data: assignments }, { data: pros }] =
    await Promise.all([
      supabase.from("member_contacts").select("phone, whatsapp").eq("member_id", id).maybeSingle(),
      supabase
        .from("consultations")
        .select("id, type, meeting_status, report_status, scheduled_at, mode, meeting_link")
        .eq("member_id", id)
        .is("cycle_id", null),
  ...
  const consultations = [...(consults ?? [])].sort(
  ...
  return (
    <section className="mx-auto max-w-4xl space-y-5">
      ...
      {/* Consultation checklist (§10 dual chips + schedule + mark done) */}
      <Card>
        <CardHeader>
          ...
          {consultations.length === 0 ? (
            ...
            consultations.map((c) => (
              <div key={c.id} className="rounded-lg border p-3">
                <div className="flex flex-wrap items-center justify-between gap-2">
                  <p className="font-medium">{ROLE_LABEL[c.type]}</p>
      ...
      {/* Start Program is the Phase 7 trigger (activate_program). */}
      <Card>
        <CardContent className="flex items-center justify-between py-4">
          <div>
            <p className="font-medium">Start program</p>
            <p className="text-sm text-muted-foreground">
              Available once all initial reports are in — arrives with the cycle engine (Phase 7).
            </p>
          </div>
